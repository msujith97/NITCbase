    ...
    */
    while (block != -1)
    {
        ...
        */
        ...
        int offset = attrCat.offset;      // byte offset inside record
        int attrType = attrCat.attrType;
        /* use the attribute offset to get the value of the attribute from
           current record */
        Attribute recAttr = record[offset];

        int cmpVal = compareAttrs(recAttr,attrVal,attrType);  // will store the difference between the attributes
        ...
        */
        if (
            ...
            */
            RecId idx {block,slot};
            RelCacheTable::setSearchIndex(relId, &idx);
...
int BlockAccess::renameRelation(char oldName[ATTR_SIZE], char newName[ATTR_SIZE]){
    ...
    RecId relcatId = BlockAccess::linearSearch(RELCAT_RELID,RELCAT_ATTR_RELNAME,newRelationName,EQ);
    ...
    relcatId = BlockAccess::linearSearch(RELCAT_RELID,RELCAT_ATTR_RELNAME,oldRelationName,EQ);
    ...
    */
   RecBuffer relCatBlock(relcatId.block);
   Attribute relCatRecord[RELCAT_NO_ATTRS];
   relCatBlock.getRecord(relCatRecord,relcatId.slot);
    /* update the relation name attribute in the record with newName.
       (use RELCAT_REL_NAME_INDEX) */
    strcpy(relCatRecord[RELCAT_REL_NAME_INDEX].sVal, newName);
    // set back the record value using RecBuffer.setRecord
    relCatBlock.setRecord(relCatRecord, relcatId.slot);
    ...
    */
    ...
    for(int i=0;i<numAttrs;i++)
    {
        ...
        // Update the relName field in the Attribute Catalog record
...
int BlockAccess::renameAttribute(char relName[ATTR_SIZE], char oldName[ATTR_SIZE], char newName[ATTR_SIZE]) {
    ...
    // 2. Search for the relation in the Relation Catalog
    RecId relcatRecId = BlockAccess::linearSearch(RELCAT_RELID, RELCAT_ATTR_RELNAME, relNameAttr, EQ);
    if (relcatRecId.block == -1 && relcatRecId.slot == -1) {
        return E_RELNOTEXIST;
    }

    // 3. Reset the searchIndex of the Attribute Catalog to start a fresh search
    RelCacheTable::resetSearchIndex(ATTRCAT_RELID);

    RecId attrToRenameRecId{-1, -1};
    Attribute attrCatEntryRecord[ATTRCAT_NO_ATTRS];

    /* 4. Iterate over all Attribute Catalog records for this relation.
       We must find oldName AND ensure newName doesn't already exist.
    */
    while (true) {
        RecId attrRecId = BlockAccess::linearSearch(ATTRCAT_RELID, ATTRCAT_ATTR_RELNAME, relNameAttr, EQ);

        // If no more attributes are found for this relation, exit loop
        if (attrRecId.block == -1 && attrRecId.slot == -1) {
            break;
        }

        // Fetch the current attribute record
        RecBuffer attrCatBlock(attrRecId.block);
        attrCatBlock.getRecord(attrCatEntryRecord, attrRecId.slot);

        // Case A: This record matches the attribute we want to rename
        if (strcmp(attrCatEntryRecord[ATTRCAT_ATTR_NAME_INDEX].sVal, oldName) == 0) {
            attrToRenameRecId = attrRecId;
        }

        // Case B: This record already has the 'newName' (collision)
        if (strcmp(attrCatEntryRecord[ATTRCAT_ATTR_NAME_INDEX].sVal, newName) == 0) {
            return E_ATTREXIST;
        }
    }

    // 5. If after checking all attributes, the oldName was not found
    if (attrToRenameRecId.block == -1 && attrToRenameRecId.slot == -1) {
        return E_ATTRNOTEXIST;
    }

    // 6. Perform the actual rename
...
{
    RecId prevRecId;
    int sid=RelCacheTable::getSearchIndex(relId,&prevRecId);
    if(sid!=SUCCESS) return sid;
