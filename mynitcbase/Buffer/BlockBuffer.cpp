...
int RecBuffer::getRecord(union Attribute *rec, int slotNum) {
  ...
  */
  ...
  unsigned char *slotPointer = buffer+HEADER_SIZE+slotCount+slotNum*recordSize /* calculate buffer + offset */;
...
int BlockBuffer::loadBlockAndGetBufferPtr(unsigned char **buffPtr) {
  ...
  {
    /* If present, update LRU: set current buffer timestamp to 0 
           and increment others */
...
int RecBuffer::setRecord(union Attribute *rec, int slotNum) {
    ...
    if (ret != SUCCESS) {
        // This point is theoretically unreachable if loadBlockAndGetBufferPtr succeeded
        return ret; 
    }
...
int BlockBuffer::setBlockType(int blockType){
    ...
    *((int32_t *)bufferPtr) = blockType;
    ...
    if(this->blockNum<0 || this->blockNum>=DISK_BLOCKS) return E_OUTOFBOUND;
...
int BlockBuffer::getFreeBlock(int blockType){
    ...
    {
      if(StaticBuffer::blockAllocMap[i]==(unsigned char)UNUSED_BLK)
      {
        freeblock = i;
        break;
      }
    ...
    int freebuf = StaticBuffer::getFreeBuffer(freeblock);
    ...
    head.numSlots=0;
    head.numEntries=0;
    int res=BlockBuffer::setHeader(&head);
    if(res!=SUCCESS) return res;

    // update the block type of the block to the input block type using setBlockType().
    int ret=BlockBuffer::setBlockType(blockType);
