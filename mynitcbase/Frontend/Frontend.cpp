...
int Frontend::select_attrlist_from_table_where(char relname_source[ATTR_SIZE], char relname_target[ATTR_SIZE],
    ...
    // 1. Create a temporary relation ".temp" using Algebra::select
    // This filters the rows based on the WHERE condition.
    int status = Algebra::select(relname_source, (char *)TEMP, attribute, op, value);

    if (status != SUCCESS) {
        return status;
    }

    // 2. Open the TEMP relation to prepare for projection
    int tempRelId = OpenRelTable::openRel((char *)TEMP);
    
    if (tempRelId < 0) {
        // If open fails, clean up the disk by deleting the temporary relation
        Schema::deleteRel((char *)TEMP);
        return tempRelId; // Returns the error code (e.g., E_RELEXIST, E_CACHEFULL)
    }

    // 3. Create the actual target relation using Algebra::project
    // This takes the filtered rows from TEMP and keeps only specified columns.
    status = Algebra::project((char *)TEMP, relname_target, attr_count, attr_list);

    // 4. Cleanup: Close and Delete the TEMP relation
    OpenRelTable::closeRel(tempRelId);
    Schema::deleteRel((char *)TEMP);

    // 5. Return the status of the projection (or the final result)
    return status;
}
