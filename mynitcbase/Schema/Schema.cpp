...
int Schema::deleteRel(char *relName) 
{
    ...
    //     return E_NOTPERMITTED
    if(strcmp(relName,"RELATIONCAT")==0||strcmp(relName,"RELATIONCAT")==0)
      return E_NOTPERMITTED;
        // (check if the relation names are either "RELATIONCAT" and "ATTRIBUTECAT".
        // you may use the following constants: RELCAT_RELNAME and ATTRCAT_RELNAME)
    int relid=OpenRelTable::getRelId(relName);
...
int Schema::createRel(char relName[], int nAttrs, char attrs[][ATTR_SIZE], int attrtype[])
{
    ...
    targetRelId = BlockAccess::linearSearch(RELCAT_RELID,
                                            (char*)"RelName",
                                            relNameAsAttribute,
                                            EQ);
    ...
    // Create relation catalog record
    ...
    int slots=(2016 / (16 * nAttrs + 1));
    relCatRecord[RELCAT_NO_SLOTS_PER_BLOCK_INDEX].nVal =slots;
    ...
    // Insert attribute records into attrcat
    for (int i = 0; i < nAttrs; i++)
    {
  ...
  // Update attribute catalog cache RelCatEntry attrCatBuf; RelCacheTable::getRelCatEntry(ATTRCAT_RELID, &attrCatBuf); attrCatBuf.numRecs += nAttrs; RelCacheTable::setRelCatEntry(ATTRCAT_RELID, &attrCatBuf);
    return SUCCESS;
}
